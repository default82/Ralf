...
1. **Self-Installer (`install.sh`)** – lädt das Repository herunter oder aktualisiert es und ruft anschließend den internen
   ...
   Branch als Standard. Erkennt er dabei eine GitHub-HTTPS-URL, wechselt er automatisch zur SSH-Variante, sodass vorhandene
   SSH-Keys ohne Benutzer-/Passworteingabe genutzt werden. Beim separaten Download empfiehlt es sich, `--repo-url` explizit auf das gewünschte Repository zu

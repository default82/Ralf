...
| `preflight`   | Führt `scripts/preflight.sh` auf dem Proxmox-Host aus und erstellt einen Hardware/Software-Bericht unter `logs/`. |
...
`ralfctl` fungiert als CLI-Wrapper und delegiert die Make-Targets für eine einheitliche Nutzererfahrung. Der Preflight-Lauf erzeugt zusätzlich einen zeitgestempelten Snapshot (`logs/preflight-report-*.txt`) mit Hardwaredaten, Storage-Zustand und Proxmox-Ressourcenübersichten.

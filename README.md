...
`origin`-URL sowie den aktuellen Branch als Standardwerte. Erkennt er dabei eine GitHub-HTTPS-URL,
wandelt er sie automatisch in die SSH-Variante (`git@github.com:<org>/<repo>.git`) um, damit eine
Authentifizierung via SSH-Key ohne Passwortabfrage erfolgt. Beim losgelösten Download (wie im obigen

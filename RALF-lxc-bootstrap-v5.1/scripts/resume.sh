...
phase2(){
  ...
  rm -f "$RESUME_FLAG"
  systemctl disable ralf-bootstrap-resume.service || true
  echo "[OK] RALF v5.1 abgeschlossen."
}
main(){
  require_root
  if ! command -v pveversion >/dev/null 2>&1; then echo "PVE fehlt"; exit 1; fi
  pkg whiptail jq curl lshw pciutils ca-certificates
  RESUME_FLAG=$(jq -r '.resume_flag' "$CONFIG_FILE")
  phase2
}
require_root(){ [[ $EUID -eq 0 ]] || { echo "Bitte als root ausführen."; exit 1; }; }
pkg(){ apt-get update -y; DEBIAN_FRONTEND=noninteractive apt-get install -y "$@"; }
